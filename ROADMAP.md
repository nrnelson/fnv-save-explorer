...
`setcondition`, `names`, `notes`, `perks`, `addictions`, `reputation`, `setreputation`, `player`, `setlevel`, `caps`, `setcaps`, `karma`, `xp`, `setkarma`, `setxp`, `limbs`, `setlimb`, `repairlimbs`, `effects`, `gdwalk`, `setglobal`, `diff`, plus
...
- **`src/FnvSaveExplorer.Core`** (`net10.0`, UI-agnostic)
  ...
  - `ChangeActorPayload.cs` — the player CHANGE_ACTOR (`0x0A`) record decoder: a changeFlags-gated section walk (MOVE / added-spell list / SPECIAL / name / package-data gap) behind the addiction reader (§4n).
  - `ChangeFormPayload.cs` — the labeled full-walk (`cfwalk`) renderer over every change-form type (§6 #1b).
...
| Player limb condition decode + edit (§4n) | ✅ 6 × float32 at actor-value **slots 180–185** of the PlayerRef+1 record (same fixed-slot mechanism as karma/XP), order `[Torso, LArm, RArm, LLeg, RLeg, Head]`, scale `0` full / `-58` healed / `-100` crippled. Cracked by Beadley cripple/heal controlled diffs; **cross-character controlled-confirmed** (Mace Windu left-leg cripple → slot 183 at -100). `PlayerLimbConditions`/`TrySetLimbCondition`/`TryRepairAllLimbs`; CLI `limbs`/`setlimb`/`repairlimbs`; same-length splice, round-trips; ground-truth-pinned tests |
...
| Player active actor-value modifiers (§4n) | ✅ the dense actor-value array's **low region (slot = AV-index, 0–76)** holds the actor's current active-effect modifier per AV (the high region is slot = AV+77 — karma/XP/skills/limbs). Cracked by `chempre`→`chempost` (a Jet) **cross-checked against the masters**: slot 12 = `0→15` matches Jet's `ALCH` effect exactly (actorValue 12 = Action Points, magnitude 15). `FalloutSave.PlayerActiveEffects` + `ReferenceChangeForm.ActorValueName`; CLI `effects` + `player` summary + GUI Body tab. Read-only (engine-recomputed). Jet ground-truth + corpus-sanity tests |
| Player CHANGE_ACTOR decode + addiction reader (§4n) | ✅ the player's `0x0A` CHANGE_ACTOR record (`iref = playerBase 0x07 + 1`) decoded as an **ordered changeFlags-gated section walk** (`Core/ChangeActorPayload.cs`: MOVE [bit1] / added-spell list / SPECIAL / name / package-data [bit11] gap) that **consumes the payload exactly** on every corpus player record. The "addiction list" is the actor's **added-spell list** (`SPEL` refs via the `−1` index); **addictions = SPIT type 10** ("X Withdrawal"), the rest are traits/abilities. Cracked by the `beadley-addiction-*` controlled FIFO (Buffout/Alcohol/Med-X, FIFO add+remove). `FalloutSave.PlayerActorSpells`/`PlayerActorChangeForm`; `TesPlugin` indexes `SPEL` + `PluginDatabase.IsAddiction`/`SpellType`; CLI `addictions` + `cfwalk` labeled sections; GUI Addictions tab. Read-only. FIFO + masters-filter + synthetic + corpus full-length-walk tests |
...
| WPF GUI (metadata, screenshot, plugins, stats, SPECIAL + skills + inventory + caps + karma/XP edit + full notes read/unread + media type + **Perks** §4n + **Addictions** §4n + **Reputation** §4o + **Globals** §4c (editable) + **Change Forms** full-walk viewer §6 #1b/#6 tabs; Body tab shows **active AV modifiers** §4n; **screenshot PNG export**; inventory **condition %** column) | ✅ launches + builds |
...
1. **Full per-type change-form payload decode (PRIMARY).** The skeleton is solved — we walk every
   ...
   **controlled-confirmed**: a Mace Windu left-leg cripple lands on the same slot 183 at the same -100 floor (+ Nathan
   ...
   "havok-dependent offset" worry was a measurement error — slots are fixed, like karma's.)* **Active actor-value
   modifiers are now DECODED + read (✅ shipped):** the dense array's **low region (slot = AV-index, 0–76)** holds the
   actor's current per-AV active-effect modifier (the high region is slot = AV+77). Confirmed by `chempre`→`chempost`
   (a Jet) cross-checked against the masters: slot 12 = `0→15` exactly matches Jet's `ALCH` effect (actorValue 12 =
   Action Points, magnitude 15). `FalloutSave.PlayerActiveEffects` + CLI `effects` + GUI Body tab; read-only (§4n).
   The **player CHANGE_ACTOR record** (`iref = playerBase+1`, formType 0x0A) is now **field-DECODED** as a
   changeFlags-gated section walk (`Core/ChangeActorPayload.cs`: MOVE[bit1]/added-spell list/SPECIAL/name/package
   data[bit11]) that consumes the payload exactly on every corpus player record, **shipping the addiction reader**:
   the added-spell list (`[count×4][7C] + refs[3 BE][7C] + [00][7C]`) holds `SPEL` refs (via the `−1` index) —
   **addictions = SPIT type 10**, the rest traits/abilities. `FalloutSave.PlayerActorSpells`; CLI `addictions` + GUI.
   (Distinct from §6 #5's deferred zero-perk `AddPerk`, whose perk list lives in the player *reference* record, not
   this base-actor record.) **Remaining is mostly SEMANTICS (needs controlled diffs):** name the sized types (`0x20`–`0x32`,

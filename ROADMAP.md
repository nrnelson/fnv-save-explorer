...
```
**Count lags the iref by one slot.** The `count` u32 stored in an entry holds the **previous** item's
stack count, not its own — confirmed against a real save with a known inventory (Save 22: the value
beside *Stimpak* was really *10mm Round*'s ×50, *Stimpak*'s true ×10 sat in the next entry; all of
Stimpak ×10 / Super Stimpak ×3 / Doctor's Bag ×3 / Weapon Repair Kit ×4 / Bleak Venom ×5 line up only
after shifting). So the decoder reads each item's count (and edit offset) from the **following** entry;
the trailing stack (a single unique/quest item in practice) falls back to its own slot. An earlier
drop-1 diff (Save 26→27) showed a `count` go 9→8 — true, but offset-only, so it didn't reveal the
mis-pairing. **Decoder:** parse the record's longest contiguous run of entries, requiring iref ≠ 0 that
resolves, both `0x7C` delimiters, and a sane count whose upper bytes aren't the `0x7C` delimiter (which
rejects misaligned reads of a stack's extra data), then shift counts by one slot. Editing a count is a
**safe same-length splice**. Names now resolve via the ESM masters (§4h).
...
| Player inventory decode + edit (§4g) | ✅ located via PlayerRef iref+1; `[iref][7C][u32 count][7C]` entries (count **lags one slot** — fixed); same-length count edit round-trips; verified vs a known real inventory (Save 22) |
...
2. ~~**Inventory**~~ — ✅ **DONE** (§4g). Cracked via a controlled drop-1 diff: items live in a dedicated
   ...
   Stack counts **lag the iref by one slot** (§4g) — fixed (counts read from the following entry), verified
   against Save 22's known inventory. Remaining nuance: per-stack **extra data** (condition / equip / mods)
   isn't decoded yet; editing targets the first stack of a given FormID (duplicate-FormID stacks are
   ambiguous by FormID alone); and a few stacks still mis-read as placed references (ACHR/ACRE/REFR → `?`)
   while a real item or two can be missed (e.g. Save 22's Antivenom) — the per-stack-imprecision item below.

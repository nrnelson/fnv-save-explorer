...
`setcondition`, `names`, `notes`, `perks`, `addictions`, `reputation`, `setreputation`, `player`, `setlevel`, `caps`, `setcaps`, `karma`, `xp`, `setkarma`, `setxp`, `limbs`, `setlimb`, `repairlimbs`, `gdwalk`, `setglobal`, `diff`, plus
...
| WPF GUI (metadata, screenshot, plugins, stats, SPECIAL + skills + inventory + caps + karma/XP edit + full notes read/unread + media type + **Perks** §4n + **Addictions** §4n + **Reputation** §4o + **Globals** §4c (editable) + **Change Forms** full-walk viewer §6 #1b/#6 tabs; **screenshot PNG export**; inventory **condition %** column) | ✅ launches + builds |
